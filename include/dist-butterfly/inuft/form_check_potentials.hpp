...
*/
#pragma once
#ifndef DBF_INUFT_FORM_CHECK_POTENTIALS_HPP
#define DBF_INUFT_FORM_CHECK_POTENTIALS_HPP
        ...
        WeightGrid<R,1,q>& weightGrid )
{
    ...
    const size_t log2NumMergingProcs = plan.GetLog2NumMergingProcesses( level );
    ...
    for( size_t cLocal=0; cLocal<(1u<<(1-log2NumMergingProcs)); ++cLocal )
    {
        ...
        WeightGrid<R,2,q>& weightGrid )
{
    ...
    vector<R> postscalingArguments( q );
    vector<R> realPostscalings( q ), imagPostscalings( q );
    for( size_t cLocal=0; cLocal<(1u<<(2-log2NumMergingProcs)); ++cLocal )
    {
        const size_t interactionIndex = parentInteractionOffset + cLocal;
        const size_t c = plan.LocalToClusterSourceIndex( level, cLocal );
        const WeightGrid<R,d,q>& oldWeightGrid = 
        ...
        WeightGrid<R,d,q>& weightGrid )
{
    ...
    vector<R> postscalingArguments( q );
    vector<R> realPostscalings( q ), imagPostscalings( q );
    for( size_t cLocal=0; cLocal<(1u<<(d-log2NumMergingProcs)); ++cLocal )
    {
        const size_t interactionIndex = parentInteractionOffset + cLocal;
        const size_t c = plan.LocalToClusterSourceIndex( level, cLocal );
        const WeightGrid<R,d,q>& oldWeightGrid = 
...
#endif // ifndef DBF_INUFT_FORM_CHECK_POTENTIALS_HPP
