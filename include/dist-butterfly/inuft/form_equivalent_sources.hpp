...
*/
#pragma once
#ifndef DBF_INUFT_FORM_EQUIVALENT_SOURCES_HPP
#define DBF_INUFT_FORM_EQUIVALENT_SOURCES_HPP
        ...
        WeightGridList<R,1,q>& weightGridList )
{
         ...
         tIndex<(1u<<log2LocalTBoxes); ++tIndex, AWalker.Walk() )
    {
             ...
             sIndex<(1u<<log2LocalSBoxes); ++sIndex, BWalker.Walk() )
        {
            ...
            {
                const vector<R>& realInverseMap = context.GetRealInverseMap(0);
                const vector<R>& imagInverseMap = context.GetImagInverseMap(0);
                // Form the real part
                Gemv
                ( 'N', q, q,
        ...
        WeightGridList<R,2,q>& weightGridList )
{
         ...
         tIndex<(1u<<log2LocalTBoxes); ++tIndex, AWalker.Walk() )
    {
             ...
             sIndex<(1u<<log2LocalSBoxes); ++sIndex, BWalker.Walk() )
        {
            ...
            // Transform with the inverse
            {
                const vector<R>& realInverseMap = context.GetRealInverseMap(1);
                const vector<R>& imagInverseMap = context.GetImagInverseMap(1);
                // Form the real part
                Gemm
                ( 'N', 'T', q, q, q,
                  R(1), &realTempWeights[0], q,
         ...
         tIndex<(1u<<log2LocalTBoxes); ++tIndex, AWalker.Walk() )
    {
             ...
             sIndex<(1u<<log2LocalSBoxes); ++sIndex, BWalker.Walk() )
        {
            ...
            for( size_t j=2; j<d; ++j )
            {
                ...
                const vector<R>& realInverseMap = context.GetRealInverseMap(j);
                const vector<R>& imagInverseMap = context.GetImagInverseMap(j);
...
#endif // ifndef DBF_INUFT_FORM_EQUIVALENT_SOURCES_HPP 
